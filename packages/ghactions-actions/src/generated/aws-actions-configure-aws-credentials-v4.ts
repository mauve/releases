...
export interface AwsActionsConfigureAwsCredentialsV4Inputs {
   ...
   * an IAM role and configure the Actions environment with the assumed role credentials rather
   * than with the provided credentials.
   */
  ...
  /**
   * Whether to export credentials as environment variables. If you set this to false, you
   * probably want to use output-credentials.
   *
   * @default true
   */
   ...
   * will retry fetching credentials until the secret access key does not contain special
   * characters. This option overrides disable-retry and retry-max-attempts. This option is
   * disabled by default
   */
  "special-characters-workaround"?: string | boolean | number;
  /**
   * When enabled, this option will check if there are already valid credentials in the
   * environment. If there are, new credentials will not be fetched. If there are not, the action
   * will run as normal.
   */
 ...
 */
export function awsActionsConfigureAwsCredentialsV4(
  inputs: AwsActionsConfigureAwsCredentialsV4Inputs,
  opts: ActionStepOptions = {},
): UsesStep {
  return makeAction("aws-actions/configure-aws-credentials@v4", inputs as unknown as Record<string, string | boolean | number | undefined>, opts);
}
